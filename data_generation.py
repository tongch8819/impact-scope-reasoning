...
def get_test_vul(save_path, category=None):
    with open(save_path, 'w') as f:
        ...
        if category:
            vuls = graph.run("match (n:VUL{category:'" + category + "'}) return distinct n order by n.NVD_ID").data()
        else:
            vuls = graph.run("match (n:VUL) return distinct n order by n.NVD_ID").data()

        for item in vuls:
            f.write('VUL$' + str(item['n'].identity) + '$' + item['n']['NVD_ID'] + '\n')
