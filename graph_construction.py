...
def generate_graph(save_path, category=None, vul_limit=None, cpe_limit=None, baseon_cpe_limit=None, with_additional_basedon=False):
    ...
    # vuls = graph.run("match (n:VUL)-[:`Scope Of Influences`]->(c:CPE) where c.cpe23Uri=~'cpe:2.3:o:linux:linux_kernel.*' return distinct n").data()
    vuls = graph.run("match (n:VUL) where n.NVD_ID=~'^CVE-2020-.*' return distinct n").data()
    ...
    for vul_id in vul_ids:
        tuples = graph.run("match (v:VUL)-[r:`Scope Of Influences`]->(cpe:CPE) where id(v)=" + str(vul_id) + " return distinct v,r,cpe").data()
    ...
    vul_cwe_tuples = graph.run("match (v:VUL)-[r:CausedBy]->(cwe:CWE) where id(v) in "+str(vul_ids)+" return distinct v,r,cwe").data()
    print('vul_cwe_tuples num: ', len(vul_cwe_tuples))
    cwe_ids = list(set([item['cwe'].identity for item in vul_cwe_tuples]))

    cpe_vendor_tuples = graph.run("match (cpe:CPE)<-[r:Develop]-(vendor:vendor) where id(cpe) in "+str(cpe_ids)+" return distinct vendor,r,cpe").data()
    ...
    cpe_cpe_tuples = graph.run("match (cpe1:CPE)-[r:BasedOn]->(cpe2:CPE) where id(cpe1) in " + str(cpe_ids) + " and id(cpe2) in " + str(cpe_ids) + " return distinct cpe1,r,cpe2").data()
    print('cpe_cpe_tuples num: ', len(cpe_cpe_tuples))

    cwe_cwe_tuples = graph.run("match (cwe1:CWE)-[r:ChildOf]->(cwe2:CWE) where id(cwe1) in " + str(cwe_ids) + " and id(cwe2) in " + str(cwe_ids) + " return distinct cwe1,r,cwe2").data()
    ...
    if with_additional_basedon:
        # 添加具有basedon模式的数据
        basedon_data = graph.run('match (v:VUL)-[r1:`Scope Of Influences`]->(cpe1:CPE)-[r0:BasedOn]->(cpe2:CPE)<-[r2:`Scope Of Influences`]-(v:VUL) return distinct v,cpe1,cpe2,r0,r1,r2').data()
        ...
        _basedon_vul_cpe_tuples_all = graph.run("match (v:VUL)-[r:`Scope Of Influences`]->(cpe:CPE) where id(v) in " + str(basedon_vul_ids) + " return distinct v,r,cpe").data()
...
def generate_graph_from_file(file_path, save_path, loc_path=None, vul_limit=None, cpe_limit=None):
    neo4j_url = 'bolt://192.168.8.48:5052'
    # neo4j_url = 'bolt://127.0.0.1:5052'
    graph = py2neo.Graph(neo4j_url, username="neo4j", password="123456")
    ...
    #     vul_loc_data = json.load(f)

    vul_cves = cve_cpe_data.keys()
    vuls = [graph.run("match (v:VUL{NVD_ID:'" + cve + "'}) return distinct v").data()[0] for cve in cve_cpe_data.keys()]
    ...
    if vul_limit:
        vuls = random.sample(vuls, vul_limit)
        print('random selected vuls num: ', len(vuls))
    vul_ids = list(set([item['v'].identity for item in vuls]))
    ...
    for vul in vuls:
        ...
        for cpe in cve_cpe_data[cve]:
            try:
                tuple = graph.run("match (v:VUL{NVD_ID:'" + cve + "'})-[r:`Scope Of Influences`]->(cpe:CPE{cpe23Uri:'" + cpe + "'}) return distinct v,r,cpe").data()[0]
                tuples.append(tuple)
    ...
    vul_cwe_tuples = graph.run(
        "match (v:VUL)-[r:CausedBy]->(cwe:CWE) where id(v) in " + str(vul_ids) + " return distinct v,r,cwe").data()
    print('vul_cwe_tuples num: ', len(vul_cwe_tuples))
    cwe_ids = list(set([item['cwe'].identity for item in vul_cwe_tuples]))
    ...
    # for item in vul_cpe_tuples:
    #     cve = item['v']['NVD_ID']
    #     if cve in vul_loc_data:
    #         for level, names in vul_loc_data[cve].items():
    #             for name in names:
    #                 if last_cve != cve:
    #                     vul_loc_tuples.append({'v': item['v'], 'r': 'ExistIn', 'loc': {'name': name, 'level': level}})
    #                     print('current cve: ', cve)
    #                     print('current vul_loc_tuples num: ', len(vul_loc_tuples))
    #                     print('current cpe_loc_tuples num: ', len(cpe_loc_tuples))
    #                 cpe_loc_tuples.append({'cpe': item['cpe'], 'r': 'BuildIn', 'loc': {'name': name, 'level': level}})
    #     last_cve = cve
    #
    # print('vul_loc_tuples num: ', len(vul_loc_tuples))
    # print('cpe_loc_tuples num: ', len(cpe_loc_tuples))

    # vul_loc_tuples = graph.run(
    #     "match (v:VUL)-[r:ExistIn]->(loc:Location) where id(v) in " + str(vul_ids) + " return distinct v,r,loc").data()
    # print('vul_loc_tuples num: ', len(vul_loc_tuples))
    # location_ids = list(set([item['loc'].identity for item in vul_loc_tuples]))
    #
    # cpe_loc_tuples = graph.run("match (cpe:CPE)<-[r:BuildIn]-(loc:Location) where id(cpe) in " + str(
    #     cpe_ids) + " and id(loc) in " + str(location_ids) + " return distinct cpe,r,loc").data()
    # print('cpe_loc_tuples num: ', len(cpe_loc_tuples))

    cpe_vendor_tuples = graph.run("match (cpe:CPE)<-[r:Develop]-(vendor:vendor) where id(cpe) in " + str(
        cpe_ids) + " return distinct vendor,r,cpe").data()
    print('cpe_vendor_tuples num: ', len(cpe_vendor_tuples))
    vendor_ids = list(set([item['vendor'].identity for item in cpe_vendor_tuples]))

    cpe_cpe_tuples = graph.run(
        "match (cpe1:CPE)-[r:BasedOn]->(cpe2:CPE) where id(cpe1) in " + str(cpe_ids) + " and id(cpe2) in " + str(
            cpe_ids) + " return distinct cpe1,r,cpe2").data()
    print('cpe_cpe_tuples num: ', len(cpe_cpe_tuples))

    cwe_cwe_tuples = graph.run(
        "match (cwe1:CWE)-[r:ChildOf]->(cwe2:CWE) where id(cwe1) in " + str(cwe_ids) + " and id(cwe2) in " + str(
    ...
    with open(save_path, 'w') as f:
        ...
        # for item in vul_loc_tuples:
        #     head = 'VUL$' + str(item['v'].identity) + '$' + item['v']['NVD_ID']
        #     tail = 'Location$' + item['loc']['level'] + '$' + item['loc']['name']
        #     f.write('\t'.join([head, item['r'], tail]) + '\n')
        #     f.write('\t'.join([tail, item['r'] + '_inv', head]) + '\n')
        #
        # for item in cpe_loc_tuples:
        #     head = 'Location$' + item['loc']['level'] + '$' + item['loc']['name']
        #     tail = 'CPE$' + str(item['cpe'].identity) + '$' + item['cpe']['cpe23Uri']
        #     f.write('\t'.join([head, item['r'], tail]) + '\n')
        #     f.write('\t'.join([tail, item['r'] + '_inv', head]) + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file_path', default=None, type=str, help="Path of cve and cpe data")
    parser.add_argument('-loc', '--loc_path', default=None, type=str, help="Path of location data")
    ...
    if os.path.isdir(args.save_path):
        ...
        if args.mode == 'neo4j':
            ...
            generate_graph_from_file(file_path=args.file_path, loc_path=args.loc_path, save_path=args.save_path, vul_limit=args.vul_limit)
            remove_space(args.save_path)
        else:
